# Does README's install work on a clean Ubuntu? README asks for exactly two
...
#   docker build --output type=cacheonly -f tools/ci/ubuntu.Dockerfile .
#
...
# `-t stelf-ci:ubuntu`; every step is cached by then, so only the export runs.
FROM ubuntu:24.04
...
# opam: the pinned upstream binary (noble's apt package is 2.1.5).
