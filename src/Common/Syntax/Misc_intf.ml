module type MISC = sig
  ...
  (** {2 Explicit Substitutions}
      ...
      Substitutions map de Bruijn indices to {!front} values. The key primitives
      are the identity ([id]), shift ([shift]), and inverse shift ([invShift]),
      with composition and application operations. *)
  ...
  (** [comp (s1, s2)] computes the composition {m s_1 \circ s_2}.
      ...
      {m \Gamma'' \vdash s_2 : \Gamma'} then
      {m \Gamma'' \vdash s_1 \circ s_2 : \Gamma}. *)

  val dot1 : Ast.sub -> Ast.sub
  (** [dot1 s] = [Dot (Idx 1, comp (s, shift))], the "dot-1" operation that
      extends [s] to work under one additional binder.

      Invariant: if {m \Gamma \vdash s : \Gamma'} then for all [V] such that
      {m \Gamma' \vdash V : L}, we have
      {m \Gamma, V[s] \vdash \mathrm{dot1}(s) : \Gamma', V}.
  ...
  (** [invDot1 s] = [comp (comp (shift, s), invShift)], the inverse of {!dot1}.
      ...
      Invariant: [invDot1 (dot1 s)] = [s], i.e., [invDot1 (Dot (Idx 1, s' o ^))]
      = [s']. *)
  ...
  (** [newAVar ()] creates a fresh assignable variable with no type, context, or
      constraint information. Returns [AVar (ref None)]. Used for lightweight
      unification variables during reconstruction. *)

  val newTypeVar : Ast.dctx -> Ast.exp
  (** [newTypeVar G] creates a fresh existential variable of type [Type] in
  ...
  (** [headOpt U] extracts the head of expression [U] if it is in strict normal
      form (a {!Root} at the outermost level, possibly under lambdas or tags).
      Returns [None] for non-atomic forms. Does {e not} look through {!EClo} or
      {!EVar}. *)

  val ancestor : Ast.exp -> Ast.ancestor
  (** [ancestor U] computes ancestor information for an expression [U], used in
      termination checking for definitions [d = U : A]. *)

  val defAncestor : Ast.cid -> Ast.ancestor
  (** [defAncestor d] retrieves the stored ancestor information for defined
      constant [d].
      @raise Match_failure if [d] is not a {!ConDef}. *)

  (** {2 Target Type Family Functions}

      These functions extract the head constant of the target (return) type of a
      type expression, looking through Pi-types, redexes, closures, and tags. *)

  val targetHeadOpt : Ast.exp -> Ast.head option
  (** [targetHeadOpt V] returns the head of the atomic target type of [V], or
      [None] if [V] is a kind, an object, or has a variable type. Does {e not}
      expand type definitions ({!Def}). *)

  val targetHead : Ast.exp -> Ast.head
  (** [targetHead V] extracts the head of the atomic target type of [V]. Like
      {!targetHeadOpt} but assumes [V] is a valid type with an atomic target.
  ...
  (** [targetFamOpt V] returns the {!cid} of the type family at the atomic
      target of [V], or [None]. Unlike {!targetHeadOpt}, this {e does} expand
      type definitions (following {!Def} to its definition). *)

  val targetFam : Ast.exp -> Ast.cid
  (** [targetFam V] extracts the type family {!cid} of the atomic target of [V].
      Like {!targetFamOpt} but assumes [V] is a valid type with a constant
      target.
