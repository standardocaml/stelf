...
module Make_Misc
  ...
  let defAncestor d = Ast.Anc (Some d, 1, None)

  let rec targetHeadOpt = function
    | Ast.Tag (_, u) -> targetHeadOpt u
    | Ast.Root (h, _) -> Some h
    | Ast.Pi (_, v) | Ast.Redex (v, _) | Ast.Lam (_, v) | Ast.EClo (v, _) ->
        targetHeadOpt v
    | Ast.EVar (r, _, _, _) ->
        begin match !r with Some v -> targetHeadOpt v | None -> None
        end
    | _ -> None

  let targetHead v =
    match targetHeadOpt v with Some h -> h | None -> invalid_arg "targetHead"
  ...
  let rec targetFamOpt = function
    ...
    | Ast.Pi (_, v) | Ast.Redex (v, _) | Ast.Lam (_, v) | Ast.EClo (v, _) ->
        targetFamOpt v
    | Ast.EVar (r, _, _, _) ->
        begin match !r with Some v -> targetFamOpt v | None -> None
        end
    | _ -> None
