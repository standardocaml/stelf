...
module Refl (M : sig type t end) : EQ with type t = M.t and type u = M.t = struct
  type t = M.t
  type u = M.t
  let prf = Refl
...
module Tran (AB : EQ) (BC : EQ with type t = AB.u) : EQ with type t = AB.t and type u = BC.u = struct
  type t = AB.t
  type u = BC.u
