...
module Reductio = struct
  ...
  and const_head_eq (n, n', sp, sp', tm, tm') =
    ...
    | _ -> raise (Syntax.Syntax "invariant violation")
    end
  ...
  and pp_normalize' = function
    ...
    | Shift (n, m) -> (List.tabulate (n, function x -> x), n + m)
  ...
  and match_one' = function
    ...
    | EltC (Elt (ATerm (ERoot (ev, (s : subst)))), elt) -> begin
        flex_left (ev, s, elt);
        []
      end
  ...
  (* PERF: we might be able to reject this faster if we knew a and a'
