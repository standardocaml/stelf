(** {1 STELF Lexing Primitives}
    ...
    This module provides the small lexer core used by the STELF-specific parser
    layer. A lexer value is a stateful computation over a [lexbuf]; the buffer
    itself stays simple so grammar-specific logic can live higher up. *)

type source = Interactive | File of Fpath.t [@@deriving show, eq, ord]

type pos = { source : source; line : int; column : int }
