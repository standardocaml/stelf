...
module Stelf (Twelf__0 : sig
  ...
  module ReconConDec :
    ReconCondec_intf.RECON_CONDEC with type condec = Parser.ExtConDec.condec
  ...
  module ReconMode :
    ReconMode_intf.RECON_MODE with type modedec = Parser.ExtModes.modedec
  ...
  open! struct
    ...
    let rec install1 = function
      ...
      | fileName, (Parser.TerminatesDec lterm, _) ->
          ...
          let _ = map (Timers.time Timers.terminate Reduces.checkFam) la_ in
          let _ =
