(* # 1 "src/print/PrintOmdoc.sig.ml" *)
open! Basis

(* Printing Signatures to OMDoc*)
(* Author: Florian Rabe *)
include PrintOmdoc_intf
(* signature PRINT_XML *)
  ...
  open! struct
      ...
      | g_, (I.Root (h_, s_), s), _ ->
          ...
          let _ =
            begin if l = 0 then out := !out ^ fmtCon (g_, h_)
              ...
              in
              let _ =
                begin if test && l > args then
                  out := (!out ^ "<om:OMA>") ^ nl_ind ()
                else ()
                end
              in
              out :=
                ((!out ^ fmtCon (g_, h_)) ^ fmtSpine (g_, (s_, s), args))
                ^ "</om:OMA>"
            end
          in
          !out
    ...
    and fmtSpine = function
      ...
      | g_, (I.App (u_, s_), s), args ->
          ...
          let _ =
            begin if args = 1 && spineLength s_ > 0 then
    ...
    and fmtBinder (binder, varname, varid, typ_, body_) =
      (((((((((((((((((((((((("<om:OMBIND>" ^ nl_ind ())
                        ...
                        ^ "<om:OMBVAR><om:OMATTR>")
                       ^ nl_ind ())
                      ^ begin if !namesafe then
                        ((((("<om:OMATP><om:OMS cd=\"omdoc\" \
                              name=\"notation\"/><om:OMFOREIGN \
    ...
    and fmtPresentation cid =
      ...
      in
      let presString1 = ("<presentation for=\"#" ^ name_ cid) ^ "\"" in
      let presString2 = "</presentation>" in
      (((((((((((((((((((presString1 ^ ">") ^ nl_ind ()) ^ useString1)
                     ^ useString2)
                    ^ nl_unind ())
                   ^ presString2)
                  ^ nl ())
