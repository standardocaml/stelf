module Make_Modern
  ...
  let with_uppercase : string list -> (unit -> 'a t) -> 'a t = fun names p ->
  ...
  type 'a t = 'a Parser.t
    ...
    match s with
    | [] -> ([], "")
    | x :: xs -> let (y, z) = break xs in (x :: y, z)
  ...
  let register_local_fixity (fix : Cst.fixity) (n : int) (ids : string list) : unit =
    let prec = FX.Strength n in
    let open Cst.View.Fixity in
    let fx = match !> fix with
  ...
  type operator =
    | Atom of Cst.Term.t
    | Infix_ of (FX.precedence * FX.associativity) * (Cst.Term.t * Cst.Term.t -> Cst.Term.t)
    | Prefix_ of FX.precedence * (Cst.Term.t -> Cst.Term.t)
    | Postfix_ of FX.precedence * (Cst.Term.t -> Cst.Term.t)

  let jux_op =
    Infix_ ((FX.inc FX.maxPrec, FX.Left),
            fun (f, x) -> Cst.Term.app f [x])

  let infix_op (infixity, tm) =
    Infix_ (infixity, fun (tm1, tm2) -> Cst.Term.app (Cst.Term.app tm [tm1]) [tm2])

  let prefix_op (prec, tm) =
    Prefix_ (prec, fun tm1 -> Cst.Term.app tm [tm1])

  let postfix_op (prec, tm) =
    Postfix_ (prec, fun tm1 -> Cst.Term.app tm [tm1])
  ...
  let classify (tm : Cst.Term.t) : operator =
    ...
    | Uppercase (_, (ns, name))
    | Qualified (_, (ns, name)) ->
      let fixity = match Hashtbl.find_opt local_fixity name with
        | Some fx -> fx
  ...
  module P = struct
    let rec reduce = function
      | Atom tm2 :: Infix_ (_, con) :: Atom tm1 :: p' -> Atom (con (tm1, tm2)) :: p'
      | Atom tm :: Prefix_ (_, con) :: p' -> Atom (con tm) :: p'
      | Postfix_ (_, con) :: Atom tm :: p' -> Atom (con tm) :: p'
      | p -> failwith (Printf.sprintf "process_app: cannot reduce stack of length %d" (List.length p))

    let rec reduce_rec = function
      | [ Atom e ] -> e
    ...
    let rec shift (opr, p) =
      ...
      | Infix_ _, Infix_ _ :: _ -> raise (ParseError "Consecutive infix operators")
      | Infix_ _, Prefix_ _ :: _ -> raise (ParseError "Infix operator following prefix operator")
      | Infix_ _, [] -> raise (ParseError "Leading infix operator")
      | (Prefix_ _ as o), (Atom _ :: _ as p') -> o :: jux_op :: p'
      | Postfix_ _, Infix_ _ :: _ -> raise (ParseError "Postfix operator following infix operator")
      | Postfix_ _, Prefix_ _ :: _ -> raise (ParseError "Postfix operator following prefix operator")
    ...
    let rec resolve (opr, p) =
      ...
      | (Infix_ ((prec, assoc), _) as o),
        ...
        | _ -> raise (ParseError "Ambiguous: infix following infix of identical precedence")
        end
      | (Infix_ ((prec, _), _) as o),
        (Atom _ :: Prefix_ (prec', _) :: _ as p') ->
        begin match FX.compare (prec, prec') with
        | Greater -> shift (o, p')
        | Less -> resolve (o, reduce p')
        | Equal -> raise (ParseError "Ambiguous: infix following prefix of identical precedence")
        end
      | (Prefix_ _ as o), p' -> shift (o, p')
      | (Postfix_ (prec, _) as o),
        (Atom _ :: Prefix_ (prec', _) :: _ as p') ->
        begin match FX.compare (prec, prec') with
        | Greater -> reduce (shift (o, p'))
        | Less -> resolve (o, reduce p')
        | Equal -> raise (ParseError "Ambiguous: postfix following prefix of identical precedence")
        end
      | (Postfix_ (prec, _) as o),
        (Atom _ :: Infix_ ((prec', _), _) :: _ as p') ->
        begin match FX.compare (prec, prec') with
        | Greater -> reduce (shift (o, p'))
        | Less -> resolve (o, reduce p')
        | Equal -> raise (ParseError "Ambiguous: postfix following infix of identical precedence")
        end
  ...
  and parse_id () : Cst.Term.t t =
    (keyword "val" *> commit
     *> (inside "(" ")" (let@ (ns, s, e) = many1 ident1 in
                         let loc = mk_loc s e in
                         let rec split = function
                           | [] -> failwith "Expected qualified name"
                           | [ p ] -> ([], p)
                           | p :: q -> let q0, q1 = split q in (p :: q0, q1)
                         in
    ...
    (let@ (name, s, e) = ident1 in
     ...
     in
     return (if is_upper || List.mem name !currently_uppercase
             then Cst.Term.uppercase ~fc:loc ([], name)
  ...
  and parse_expr_trail () : Cst.Term.t t =
    ...
    <|> ((let* ids = inside "{{" "}}" (many @@ parse_var ()) in 
          ...
          body) <?> "expression with implicit variables")
    ...
    begin
      (
          ...
          *>
          ...
          (keywords ["->"; "if"] *> commit
          *>
          let+ args = sep_by1 (option () @@ keyword "->" *> commit) (parse_expr1 ()) in
          ...
          *>
          ...
          (let* atoms = many @@ parse_expr1 () in
           let* trail_opt = option None (let+ t = parse_expr_trail () in Some t) in
           ...
           | head :: rest, Some trail -> return (process_app (head :: (rest @ [ trail ]))))
      )
    end
    <?> "expression"
