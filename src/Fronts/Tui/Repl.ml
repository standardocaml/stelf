...
let () = Debug.setup_log ~level:(Debug.Level.log_level @@ Debug.Level.from_chatter @@ Display.Info.to_int M.verbosity) ()
let () = Display.setup' ()   

let seq_l : 'a 'b . 'a Lwt.t -> 'b Lwt.t -> 'b Lwt.t = fun a b ->
...
type response = Continue | Fail of string | Stop 
  ...
  let rec read : (string -> response Lwt.t) -> int Lwt.t = fun f ->
    let open Lwt.Syntax in
    let* term' = term in 
    ...
    let display_err (m : Display.Info.t) : unit Lwt.t =  
      ...
      let out = Form.(markup @@ concat [arrow; sp (); m.msg]) in
      LTerm.printls out
    in 
    let display_warn (m : Display.Info.t) : unit Lwt.t = let open Display in 
      let arrow = Form.(styles Style.[Fore.red; bold] (string "!!")) in 
      let out = Form.(markup @@ concat [arrow; sp (); m.msg]) in
      LTerm.printls out
    in 
    let display_info (m : Display.Info.t) : unit Lwt.t = let open Display in 
      let arrow = Form.(styles Style.[Fore.red; bold] (string "*>")) in 
      let out = Form.(markup @@ concat [arrow; sp (); m.msg]) in
      LTerm.printls out
    in 
    let display_debug (m : Display.Info.t) : unit Lwt.t = let open Display in 
      let arrow = Form.(styles Style.[Fore.red; bold] (string "?>")) in 
      let out = Form.(markup @@ concat [arrow; sp (); m.msg]) in
      LTerm.printls out
    in 
    let display_response (m : Display.Info.t) : unit Lwt.t = let open Display in 
      let arrow = Form.(styles Style.[Fore.green; bold] (string "=>")) in 
      let out = Form.(markup @@ concat [arrow; sp (); m.msg]) in
      LTerm.printls out
    in 
     
    let should_display (msg : Display.Info.t) : bool =  
       Display.Info.(>=) M.verbosity msg.level in 
    let display : Display.Info.t -> unit Lwt.t = fun m -> if should_display m then 
      Display.Info.(match m.kind with 
    ...
    let* continue = f r0 in
    
    match continue with
    | Continue -> seq_l (flush ()) (read f)
    | Fail msg -> Printf.eprintf "Error: %s\n%!" msg; seq_l (flush ()) (Lwt.return 1)
