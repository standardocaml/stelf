...
include CsEqIntegers_intf
(* signature CS_EQ_FIELD *)

(* # 1 "src/solvers/CsEqIntegers.fun.ml" *)
open! Basis
...
struct
  ...
  module Integers = CSEqIntegers__0.Integers
  module Unify = CSEqIntegers__0.Unify
  ...
  open! struct
    ...
    and sameSub = function
      ...
      | (Dot (Idx _, _) as s1), Shift k2 ->
          let k2' = Stdlib.( + ) k2 1 in
          sameSub (s1, Dot (Idx k2', Shift k2'))
      | Shift k1, (Dot (Idx _, _) as s2) ->
          let k1' = Stdlib.( + ) k1 1 in
          sameSub (Dot (Idx k1', Shift k1'), s2)
      | _, _ -> false
...
(* # 1 "src/solvers/CsEqIntegers.sml.ml" *)
