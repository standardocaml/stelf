...
module Traverse (Traverse__0 : sig
  ...
  (*! structure IntSyn = IntSyn' !*)
  module Traverser = struct
    include Traverse__0.Traverser_param
  end
    ...
    and fromObjW = function
      ...
      | g_, (I.Lam (d_, u_), s), (I.Pi (_, v_), t) ->
          ...
          T.lam
            ( fromDec (g_, (d'_, s)),
              fromObj
                (I.Decl (g_, I.decSub (d'_, s)), (u_, I.dot1 s), (v_, I.dot1 t))
            )
...
(* shares types from Traverser_param *)
(* local ... *)
(* functor Traverse *)

(* # 1 "src/print/Traverse.sml.ml" *)
